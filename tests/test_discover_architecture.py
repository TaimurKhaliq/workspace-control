...
def test_discover_architecture_backend_repo_prefers_existing_locations(
    ...
    assert "discovered frameworks: flyway, openapi, spring_boot" in captured.out
    assert "hinted frameworks: flyway, openapi, spring_boot" in captured.out
...
def test_discover_architecture_frontend_repo_prefers_existing_locations(
    ...
    assert "discovered frameworks: react" in captured.out
...
def test_discover_architecture_metadata_only_repo_reports_hints(
    ...
    assert repo.detected_frameworks == []
    assert repo.hinted_frameworks == ["openapi", "spring_boot"]
    assert repo.likely_api_locations == []
    ...
    assert "discovered frameworks: -" in captured.out
    assert "hinted frameworks: openapi, spring_boot" in captured.out
...
def test_discover_architecture_metadata_only_frontend_framework_hint(
    tmp_path: Path, capsys
) -> None:
    repo_path = tmp_path / "web-ui"
    repo_path.mkdir(parents=True, exist_ok=True)
    ...
    assert repo.confidence == "low"
    assert repo.detected_frameworks == []
    assert repo.hinted_frameworks == ["react"]
    assert repo.likely_ui_locations == []
    assert repo.hinted_ui_locations == ["src/pages", "src/components", "src/forms"]
    ...
    assert "mode: metadata-only" in captured.out
    assert "discovered frameworks: -" in captured.out
    assert "hinted frameworks: react" in captured.out
    assert "ui/components: src/pages, src/components, src/forms" in captured.out
