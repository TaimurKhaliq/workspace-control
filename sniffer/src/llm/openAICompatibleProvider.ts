...
export class OpenAICompatibleProvider implements LlmProvider {
  ...
  async critiqueGraphStructure(context: GraphStructureCriticContext): Promise<Pick<GraphRefinementResult, 'suggestions' | 'warnings'>> {
    ...
    const prompt = [
      ...
      'Use only targetId values listed in context.targetRegistry. Do not invent control IDs, fact IDs, or surface IDs.',
      'If the best target is not listed, emit type "unresolved_observation" and explain what evidence would be needed instead of inventing an ID.',
      'When available, include targetKind and targetEvidenceIds. targetEvidenceIds may reference the source fact that owns a control/surface/workflow.',
      ...
      'Return JSON only matching this shape:',
      '{"suggestions":[{"id":"...","type":"reclassify_fact|normalize_control|merge_duplicate_surface|split_surface|add_edge|remove_edge|raise_confidence|lower_confidence|mark_as_noise|add_workflow|reclassify_surface|unresolved_observation","targetId":"...","targetKind":"fact|control|surface|workflow|action|api|edge","targetEvidenceIds":["fact-..."],"targetAlias":"...","resolutionHint":"...","fromValue":"...","toValue":"...","reason":"...","evidenceIds":["fact-..."],"confidence":"low|medium|high","risk":"low|medium|high"}],"warnings":["..."]}',
