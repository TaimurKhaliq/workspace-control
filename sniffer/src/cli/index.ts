...
function printProviderCheck(result: LlmProviderCheckResult): void {
  const lines = [
    ...
    `Vision supported: ${result.visionSupported ? 'yes' : 'no'}`,
    `Vision enabled: ${result.visionEnabled ? 'yes' : 'no'}`,
    `Image input style: ${result.imageInputStyle ?? 'none'}`,
    result.maxImageBytes !== undefined ? `Max image bytes: ${result.maxImageBytes}` : undefined,
    result.imageDetail ? `Image detail: ${result.imageDetail}` : undefined,
    `Auth configured: ${result.authConfigured ? 'yes' : 'no'}`,
    `Config source: baseUrl=${result.configSource.baseUrl ?? 'missing'} apiKey=${result.configSource.apiKey ?? 'missing'} model=${result.configSource.model ?? 'missing'} apiStyle=${result.configSource.apiStyle ?? 'missing'} vision=${result.configSource.visionEnabled ?? 'model/default'} maxImageBytes=${result.configSource.maxImageBytes ?? 'default'} imageDetail=${result.configSource.imageDetail ?? 'default'}`,
